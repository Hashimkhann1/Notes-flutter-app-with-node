  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocBuilder<NotesBloc , NotesState>(builder: (context , state) {
        return state.notesStatus == NotesStatus.loading ? Center(child: CircularProgressIndicator(color: Colors.blue,)) : state.notesList.length <= 0 ? Center(child: MyText(title: 'No notes yet!' ,fontSize: 24,fontWeight: FontWeight.w600,color: Colors.grey,)) : MasonryGridView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            return GestureDetector(
              onTap: () {
                Navigator.push(context, CupertinoPageRoute(builder: (context) => AddNewNote(isUpdate: true,noteModel: currentNoteData,)));
              },
              child: Container(
